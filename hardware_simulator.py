...
meta = {
    ...
    "models": {
        "HardwareInterface": {                                   # ハードウェアインターフェースモデル
            ...
            "params": ["device_id", "connection_type"],          # 初期化パラメータ（デバイスID、接続タイプ）
            "attrs": ["sensor_value", "actuator_command", "hardware_input", "status"],  # 公開属性
        },
    },
}
...
class HardwareSimulator(mosaik_api.Simulator):
    ...
    def step(self, time, inputs, max_advance=None):
        ...
        for eid, entity in self.entities.items():
            ...
            entity["sensor_value"] = self._read_sensor_data()      # ランダムなセンサー値を生成
            entity["hardware_input"] = entity["sensor_value"]      # センサー値をハードウェア入力として設定
            ...
            if eid in inputs:
                for attr, values in inputs[eid].items():
                    if attr == "output":                       # 数値シミュレーターの出力を受信
                        ...
                        sim_output = list(values.values())[0]  # シミュレーション出力値を取得
                        entity["actuator_command"] = sim_output # アクチュエーターコマンドとして設定
                        self._send_actuator_command(entity["actuator_command"])  # アクチュエーターにコマンド送信（シミュレート）

            entity["last_update"] = time                           # 最終更新時刻を記録
            entity["status"] = "active" if time > 0 else "initializing"  # ステータスを更新
    ...
    def _send_actuator_command(self, command):
        ...
        print(f"Hardware actuator command: {command:.3f}")  # コマンド値をコンソールに表示
