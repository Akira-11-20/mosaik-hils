...
def plot_comparison(scenarios, baseline_data, output_file, title_suffix=""):
    ...
    for i, scenario in enumerate(scenarios):
        ...
        linesty = "-" if "post" in scenario["label"] else "-."
        ax.plot(scenario["data"]["time_s"], scenario["data"]["position"], **style, linestyle=linesty)
    ...
    for i, scenario in enumerate(scenarios):
        style = get_scenario_style(i, label=scenario["label"])
        linesty = "-" if "post" in scenario["label"] else "-."
        ax.plot(scenario["data"]["time_s"], scenario["data"]["velocity"], **style, linestyle=linesty)
    ax.set_xlabel("Time [s]", fontsize=FONT_SETTINGS["label_size"], fontweight=FONT_SETTINGS["label_weight"])
...
def plot_comparison_post_only(scenarios, baseline_data, output_file, title_suffix=""):
    """label に 'post' を含むシナリオだけを描画して保存するヘルパー。"""
    # 'post' を含むシナリオだけにフィルタ
    post_scenarios = [s for s in scenarios if "post" in s["label"]]

    if not post_scenarios:
        print("post を含むシナリオが無かったので、図は保存しませんでした。")
        return
...
def main():
    ...
    baseline_dir = base_dir / "20251120-174813_baseline_rt"
    scenario_dirs = [
        {"dir": base_dir / "20251120-174846_delay0ms_pre_comp_tau50ms_linear", "tau": 50.0, "position": "pre"},
        {"dir": base_dir / "20251120-174922_delay0ms_pre_comp_tau100ms_linear", "tau": 100.0, "position": "pre"},
        {"dir": base_dir / "20251120-174959_delay0ms_pre_comp_tau150ms_linear", "tau": 150.0, "position": "pre"},
        {"dir": base_dir / "20251120-175034_delay0ms_pre_comp_tau200ms_linear", "tau": 200.0, "position": "pre"},
        {"dir": base_dir / "20251120-175110_delay0ms_post_comp_tau50ms_linear", "tau": 50.0, "position": "post"},
        {"dir": base_dir / "20251120-175146_delay0ms_post_comp_tau100ms_linear", "tau": 100.0, "position": "post"},
        {"dir": base_dir / "20251120-175223_delay0ms_post_comp_tau150ms_linear", "tau": 150.0, "position": "post"},
        {"dir": base_dir / "20251120-175259_delay0ms_post_comp_tau200ms_linear", "tau": 200.0, "position": "post"},
    ]
    ...
    plot_comparison_post_only(
        scenarios,
        baseline_data,
        base_dir / "linear_tau_sweep_comparison_post_only.png",
