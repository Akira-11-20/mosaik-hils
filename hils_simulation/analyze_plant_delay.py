...
def analyze_plant_delay(h5_file):
    ...
    if len(cmd_nonzero) > 0 and len(meas_nonzero) > 0:
        ...
        print(f"  Communication delay (command → measured):")
    ...
    if len(step_indices) > 0:
        ...
        meas_window = measured_thrust[start_idx:end_idx]
        actual_window = actual_thrust[start_idx:end_idx]
        ...
        if step_idx < len(measured_thrust) - 1:
            ...
            if abs(step_size) > 1.0:  # Significant step
                ...
                if len(crossing) > 0:
                    ...
                    print(f"  Plant time constant (estimated from step response):")
