...
with h5py.File(h5_path, 'r') as f:
    ...
    for i in range(sub_steps):
        ...
        if i < 3 or i >= sub_steps - 3:  # 最初と最後の数ステップを表示
            print(f"    サブステップ {i}: y = {y_old:.6f} + ({dt_sub:.6f}/{tau:.6f}) * ({u_current:.6f} - {y_old:.6f}) = {y:.6f}")
    ...
    for i in list(range(21)) + list(range(95, min(106, len(time_ms)))):
        print(f"{i:4d} | {time_ms[i]:8.3f} | {measured_thrust[i]:12.6f} | {actual_thrust[i]:10.6f} | {time_constant[i]:6.2f}")
