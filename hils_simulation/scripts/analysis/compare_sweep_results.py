...
def plot_sweep_comparison(baseline_data: dict, sweep_data: dict, output_dir: Path):
    ...
    sorted_cases = sorted(sweep_data.items(),
                         key=lambda x: x[1]['cmd_delay'] + x[1]['sense_delay'])

    # Filter out 200ms case
    sorted_cases = [(name, data) for name, data in sorted_cases
                   if data['total_delay'] != 200]
    ...
    for idx, (case_name, case_data) in enumerate(sorted_cases):
        ...
        ax.plot(time[:min_len], case_data['data']['position'][:min_len],
                '--', color=color, label=f'{total_delay}ms delay', linewidth=1.5)
    ...
    for idx, (case_name, case_data) in enumerate(sorted_cases):
        ...
        ax.plot(time[:min_len], pos_diff, '-', color=color,
                label=f'{total_delay}ms delay', linewidth=1.5)
...
def plot_200ms_comparison(baseline_data: dict, sweep_data: dict, output_dir: Path):
    ...
    ax.plot(time, baseline_data['position'], 'k-', label='Baseline (No Delay)', linewidth=2.0)
    ax.plot(time[:min_len], case_data['data']['position'][:min_len],
            'r--', label='200ms delay', linewidth=1.5)
    ax.set_xlabel('Time [s]')
    ax.set_ylabel('Position [m]')
...
def plot_metrics_summary(all_metrics: dict, output_dir: Path):
    ...
    delays = [f"{item[1]['cmd_delay']}+{item[1]['sense_delay']}"
...
def save_summary_report(baseline_data: dict, all_metrics: dict, output_dir: Path):
    ...
    with open(txt_file, 'w') as f:
        ...
        f.write(f"Baseline: No Delay (RT mode)\n")
