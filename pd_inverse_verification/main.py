...
class PDInverseSimulation:
    ...
    def run_with_compensation(self):
        ...
        inv_comp = inv_sim.InverseCompensator(comp_gain=self.params["comp_gain"], dt=self.params["dt"])
...
def plot_comparison(no_comp_dir, with_comp_dir, params):
    ...
    ax.plot(
        with_comp["time"],
        with_comp["control_output"],
        label="Control Output (With Comp)",
        linewidth=2,
        alpha=0.7,
        linestyle="--",
    )
    ...
    ax.plot(
        with_comp["time"], with_comp["velocity"], label="Velocity (With Comp)", linewidth=2, alpha=0.7, linestyle="--"
    )
    if with_comp["inv_output"] is not None:
        ax.plot(
            with_comp["time"], with_comp["inv_output"], label="Inv Comp Output", linewidth=2, alpha=0.7, linestyle=":"
        )
