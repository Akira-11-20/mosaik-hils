...
def add_non_gaussian_noise(
    measurement: np.ndarray, std: float, outlier_prob: float = 0.1, outlier_scale: float = 10.0
) -> np.ndarray:
    """
    非ガウスノイズ (外れ値を含む)
...
def run_comparison(
    ...
    mckf = SimpleMCKF(A, B, C, Q, R, x0_est, P0, kernel_bandwidth=kernel_bandwidth, max_iterations=10)
...
def plot_results(results: dict, save_path: Path = None):
    ...
    ax.scatter(
        time[outlier_idx],
        results["measurement"][outlier_idx],
        c="orange",
        s=30,
        marker="x",
        alpha=0.7,
    ...
    ax.plot(time[iter_idx], results["mckf_iterations"][iter_idx], "ro", markersize=3, label="MCKF Iterations")
...
def main():
    ...
    # プロット
    plot_results(results, save_path=save_dir / "mckf_simple_comparison.png")
    plt.show()
