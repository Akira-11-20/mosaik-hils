#!/usr/bin/env python3
"""
Compare compensation gain (alpha) sweep results against RT baseline.

This script:
1. Loads RT baseline and multiple compensation gain scenarios
2. Computes error metrics (RMSE, MAE, Max Error)
3. Generates comparison plots (all gain values in one plot)
...
def main():
    ...
    baseline_dir = base_dir / "20251120-143626_baseline_rt"
    scenario_dirs = [
        {"dir": base_dir / "20251120-143659_delay0ms_post_comp_tau100ms", "gain": 7},
        {"dir": base_dir / "20251120-143736_delay0ms_post_comp_tau100ms", "gain": 8},
        {"dir": base_dir / "20251120-143812_delay0ms_post_comp_tau100ms", "gain": 9},
        {"dir": base_dir / "20251120-143846_delay0ms_post_comp_tau100ms", "gain": 10},
        {"dir": base_dir / "20251120-143922_delay0ms_post_comp_tau100ms", "gain": 11},
        {"dir": base_dir / "20251120-143959_delay0ms_post_comp_tau100ms", "gain": 12},
        {"dir": base_dir / "20251120-144035_delay0ms_post_comp_tau100ms", "gain": 13},
    ]
    ...
    for scenario_info in scenario_dirs:
        ...
        gain = scenario_info["gain"]

        print(f"\nLoading gain={gain} from: {dir_path.name}")
    ...
    plot_comparison(
        scenarios,
        baseline_data,
        base_dir / "alpha_sweep_comparison.png",
        "",
    )
    ...
    with open(summary_file, "w") as f:
        f.write("=" * 80 + "\n")
        f.write("COMPENSATION GAIN (ALPHA) SWEEP COMPARISON: SUMMARY STATISTICS\n")
        f.write("=" * 80 + "\n\n")
        ...
        for scenario in scenarios:
            f.write(f"Compensation Gain α = {scenario['gain']}:\n")
    ...
    with open(csv_file, "w") as f:
        f.write("Comp_Gain_Alpha,Pos_RMSE[m],Pos_MAE[m],Pos_MaxErr[m],Vel_RMSE[m/s],Vel_MAE[m/s],Vel_MaxErr[m/s]\n")
        for scenario in scenarios:
            f.write(
                f"{scenario['gain']},"
