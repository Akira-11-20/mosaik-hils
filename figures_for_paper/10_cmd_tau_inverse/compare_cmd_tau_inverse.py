...
This script:
1. Loads RT baseline and multiple command delay scenarios with linear tau model + inverse comp
2. Computes error metrics (RMSE, MAE, Max Error)
3. Generates comparison plots (all scenarios in one plot)
...
def main():
    ...
    baseline_dir = base_dir / "20251120-161030_baseline_rt"
    scenario_dirs = [
        {"dir": base_dir / "20251120-161103_cmd10ms_sense0ms_post_comp_tau100ms_linear", "cmd_delay": 10.0},
        {"dir": base_dir / "20251120-161139_cmd20ms_sense0ms_post_comp_tau100ms_linear", "cmd_delay": 20.0},
        {"dir": base_dir / "20251120-161219_cmd30ms_sense0ms_post_comp_tau100ms_linear", "cmd_delay": 30.0},
        {"dir": base_dir / "20251120-161301_cmd40ms_sense0ms_post_comp_tau100ms_linear", "cmd_delay": 40.0},
    ]
    ...
    for scenario_info in scenario_dirs:
        ...
        print(f"\nLoading cmd_delay={cmd_delay}ms from: {dir_path.name}")
        ...
        scenarios.append(
            {
                ...
                "data": data,
                "metrics": metrics,
                "label": f"Cmd={int(cmd_delay)}ms, τ=100ms",
            }
        )
    ...
    plot_comparison(
        scenarios,
        baseline_data,
        base_dir / "cmd_tau_inverse_comparison.png",
        "",
    )
    ...
    with open(summary_file, "w") as f:
        f.write("=" * 80 + "\n")
        f.write("CMD DELAY + LINEAR TAU + LINEAR MODEL INVERSE COMPENSATION COMPARISON\n")
        f.write("=" * 80 + "\n\n")
        ...
        for scenario in scenarios:
            f.write(f"Command Delay = {scenario['cmd_delay']}ms (Linear τ + Linear Inverse Comp):\n")
    ...
    with open(csv_file, "w") as f:
        f.write("Cmd_Delay[ms],Pos_RMSE[m],Pos_MAE[m],Pos_MaxErr[m],Vel_RMSE[m/s],Vel_MAE[m/s],Vel_MaxErr[m/s]\n")
        for scenario in scenarios:
            f.write(
